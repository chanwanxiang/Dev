...
包含H5和web端，分为渠道、H5、行内3个系统，
业务流程为
渠道生成征信预审推送H5 ->
预审通过分期信息录入提交 ->
行内系统审核上送新一代系统(分期材料录入)
...
包含
字段输入(fill)、
下拉选择(page.locator(li:visible).filter().click())、
文件上传(page.expect_file_chooser())
等自动化实现
