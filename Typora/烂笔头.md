...
切片是针对 Python 有序序列（字符串、列表、元组等）的一种高效取值方式，能快速截取序列中指定范围的部分元素，生成一个新的序列，且不会修改原序列
...
==进程是程序运行的独立容器,操作系统能够调度的最小单元就是线程==
...
数据报**「不可靠、无序、有边界」的独立数据包传输**

#### 7.2 TCP总结

==TCP是一种面向连接的、可靠的、基于字节流(以「连续字节序列」的方式传输数据，无数据包边界)的传输层通信协议== ,为两台主机提供高可靠的数据通信服务.它可以将源主机的数据无差别的传输到目标主机.当有数据要发送时,对应用进程送来的数据进行分片,以适合在网络层中传输;当接收到网络层传来的分组时,要对收到的分组进行确认,还要对丢失的分组设置超时重发等.为此TCP需要增加额外的开销,以便在数据传输过程中进行必要的控制,确保数据的可靠传输.因此TCP传输效率比较低

字节流**「可靠、有序、无边界」的连续数据传输**
...
3)保持长连接
与http不同的是,websocket需要先创建连接,这就使得其成为了一种有状态的协议,而http请求可能需要在每个请求都携带状态信息(如身份认证等)
...
| 指令 |     核心定位      |                核心功能                |                   适用场景                    |      |
| :--: | :---------------: | :------------------------------------: | :-------------------------------------------: | ---- |
| grep | 文本过滤/查找工具 |          按「内容匹配」筛选行          |   快速找日志中的错误、筛选符合条件的文本行    |      |
| sed  |     流编辑器      | 按「行」编辑文本（替换 / 删除 / 新增） | 批量修改配置文件、日志清洗（简单替换 / 删行） |      |
| awk  | 文本分析/处理工具 |     按「列 / 字段」处理、数据计算      | 日志分析（提取 IP / 耗时）、数据统计、列处理  |      |
...
3. 特殊变量(位置参数变量)
   ...
   如: ./myshell.sh 100 200,这个就是一个执行shell的命令行,可以在myshell脚本中获取到参数信息
...
**主键是表的唯一标识（数据约束），外键是表之间的关联纽带（关联约束），索引是提升查询效率的工具（性能优化）**，三者的设计目的和作用完全不同。

##### ==10.1.6 数据库怎么优化查询效率?介绍一下索引,查询语句里面加一些什么索引,索引为什么能加快查询速度==

1.开启慢查询日志 2.优化手段(创建合适索引、sql语句优化、优化数据库配置)
...
**本质**：索引是「有序的快速查找数据结构」，核心是 **用空间换时间**（索引占用额外存储，增删改时需维护索引）
**核心原理**：通过 B+ 树的**有序性 + 二分查找**，将查询的时间复杂度从 O (n) 降到 O (log₂n)，减少磁盘 I/O 次数
**关键前提**：合理设计索引（避免过度索引、遵循最左前缀），否则会适得其反（增加写入开销，索引失效）

##### 10.1.7 NoSQL和关系数据库的区别?

##### 10.1.8 drop、delete、truncate三者区别？

「`DROP` 删表,`DELETE` 删行（可回滚）,`TRUNCATE` 清空（快且重置）」
...
![image-20210526170856822](https://fastly.jsdelivr.net/gh/chanwanxiang/imageHosting/img/image-20210526170856822.png)
...
def bubbleSort(sequ):
    ...
    for i in range(len(sequ)-1):
        ...
        # 在单次冒泡排序中,如果没有任意连个数交互位置,则说明不需要在进行下一轮冒泡排序,直接退出循环
        if flag == False:
            break
...
+ ###### 购物车设计测试用例
  ...
  ​	添加商品到购物车是否正常、商品数量上限、同类商品是否分类展示、排序是否合理
  ...
  ​	选中购物车商品时预结算的支付金额以及商品数量是否正确

  

  商品属性是否能够添加到购物车，根据经验秒杀商品、虚拟商品无法添加购物车，已加购物车商品下架以及库存不足展示

  

  购物车与其他模块关联登录、优惠券、折扣、订单、支付

  未登录状态下，添加商品到购物车是否跳转登录页面

  选中购物车商品生成订单，购物车商品是否清除

  领取优惠券或折扣时结算金额是否随之变化、是否可以使用不同结算方式支付
...
+ ###### 支付设计测试用例
  ...
  1. 选择商品点击支付
     ...
     1. 校验:点击支付，是否生成支付订单
     2. 校验:生成订单之后，取消订单，校验订单状态

  2. 选择支付方式

     1. 遍历不同支付方式(微信、支付宝、银行卡、网银等)
     2. 支付时未安装三方软件
     3. 支付时未绑定银行卡等
  ...
  5. 确认支付结果
     ...
     1. 支付成功
        1. 支付成功订单状态流转
        2. 支付成功发起退款，退款金额返回路径
        3. 退款涉及优惠券或红包是否返回可以继续使用
     2. 支付失败
        1. 校验支付失败场景以及对应code码的信息


##### 12.1.3 web端和App端测试的不同?
...
##### 12.1.5 如何提交一份高质量的缺陷跟踪单

步骤可复现(逐条清晰没有歧义)、信息无遗漏(环境、数据、日志)、结果明确化(实际vs预期)
标题清晰简洁、环境信息完整、详细复现步骤、明确预期结果、合理设置严重程度和优先级
...
+ 相对路径定位
  ...
  //div/input	 	定位div下的直接input子元素

  //form//input	  form后代中的所有input（跨层级）

+ 属性精确匹配

  //input[@id='search']	 								  通过id定位

  //button[@type='submit' and @class='btn']	多属性组合

###### 2. 索引文本定位

+ 索引定位

  //ul[@class='goodlist']/li[1]

  //ul[@class='goodlist']/li.last

+ 文本匹配

  //button[text()='提交订单']

  //div[contains(text(),'订单提交成功')]

  //p[normalize-space(text())='我的订单']	忽略文本前后空格

###### 3. 模糊匹配函数

| 函数        |              作用              | 语法示列                            |
| ----------- | :----------------------------: | :---------------------------------- |
| contains    |  匹配「包含指定字符串」的元素  | //div[contains(text(), '登录')]     |
| starts-with | 匹配「以指定字符串开头」的元素 | //input[starts-with(@name, 'user')] |
| ends-with   | 匹配「以指定字符串结尾」的元素 | //a[ends-with(@href, '.html')]      |

###### 4. 轴定位

| 名称      |                        范围                        | 示例                                                      |
| --------- | :------------------------------------------------: | --------------------------------------------------------- |
| ancestor  | 当前节点的所有祖先节点(父、祖父… 根节点，不含自身) | //input[@name='password']/ancestor::div[@id='login-form'] |
| parent    |           当前节点的直接父节点(等价于..)           | //input[@name='phone']/parent::div                        |
| child     |          当前节点的所有子节点(等价于 //)           | //div[@id='login-form']/child::input                      |
| following |       当前节点之后的所有节点(跨层级、不同父)       | //label[text()='姓名']/following::input[position()=1]     |

##### 12.1.8 怎么判断一个缺陷属于前端还是后端

前端缺陷常有特征
	接口返回数据正确,但页面展示不对,页面是否布局合理、字段完整、文案正确,点击按钮是否触发事件
	控制台里有报错的信息,如javascript error、资源加载失败
后端缺陷常有特征
	前端传参正确,接口返回错误
	后端日志中出现异常信息,如空指针、sql异常、权限错误等等

#### 12.2 jenkins + jmeter CI

##### 1. 使用说明

**jmeter目录说明**

![image-20210917141429741](https://fastly.jsdelivr.net/gh/chanwanxiang/imageHosting/img/image-20210917141429741.png)
...
T:使用jmeter Beanshell前置处理器对请求体进行加密操作,并使用beanshell后置处理器对请求响应中的密文进行解密并重新赋值结果树的展示结果
...
// 引入csv文件中orderNumber参数
String orderNumber = "${orderNumber}";
log.info("订单:" + orderNumber);

// 拼接需要加密的请求体文本
String request = "{\"machineNum\":\"2161AF600052\",\"bizContent\":{\"identityNo\":\"00003\",\"memNo\":\"1557927415651635200\",\"memCardNo\":\"1557927415651635200\",\"cardtype\":\"100\",\"cardno\":\"EAC6862C\",\"clientTransNo\":" + orderNumber + ",\"totalAmt\":17.23}}";
log.info(request);

// 调用jar包中加密方法对请求体文本进行加密
sign = RsaUtil.encryptContentByPublickey(request, pubkey);
log.info(sign);

// 将加密后的文本放入全局变量以供其他组件使用
vars.put("sign", sign);
````

json提取器提取响应结果中被加密文本

![image-20220817174310000](https://cdn.jsdelivr.net/gh/chanwanxiang/imageHosting/img/image-20220817174310000.png)
...
// 对json提取器中的响应结果密文进行解密
resl = RsaUtil.decryptContentByPublickey(resp, pubkey);
log.info(resl);

// prev.setResonseData()方法为结果树显示的结果重新赋值
prev.setResponseData(resl);
...
+ 并发用户数:用来度量服务器并发容量和同步协调能力,在客户端指一批用户同时执行一个操作,并发数反映了软件系统并发处理能力，系统同时处理的request/事物数

+ 响应时间(RT):客户端发出请求到得到响应的整个过程所经历的时间

  + 响应时间 = 客户端发起请求 + 网络传输时间 + 服务器处理时间

+ 吞吐量:指单位时间内系统处理的客户请求的数量,直接体现软件系统的性能承载能力

  TPS(每秒事物数)、QPS(每秒查询数)是吞吐量常用量化指标
  TPS = 并发数 / 平均响应时间
...
​    用户体验层（前端性能、页面加载时间）
​    应用服务层（接口响应时间、吞吐量、错误率）
​    中间件层（数据库、缓存、消息队列）
​    基础设施层（CPU、内存、网络、磁盘IO）
...
1. 工作中 JMeter 常用函数按场景核心分类：参数化（\__CSVRead、\__V）、随机值（\__Random、\__UUID）、时间（\__time）、字符串（\__substring）、逻辑（\__if、\__groovy）、计数器（__counter）
2. 高频组合用法:\__time + __Random生成唯一订单号，\__CSVRead + __counter逐行读取 CSV，\__V解析嵌套变量
3. 函数助手是快速生成函数的核心工具，避免手写语法错误

###### 2. 使用jmeter如何做接口之间的数据关联

将前一个接口的响应数据提取为 JMeter 变量，在后一个接口的请求参数 / 请求头中引用该变量
json提取器、正则提取器、xpath提取器

###### 3. 使用jmeter做接口测试的流程

核心流程是==需求准备→脚本设计→调试执行→结果分析→报告生成==  ,全程围绕接口功能验证、参数校验、性能摸底展开

###### 4. jmeter中有哪些参数化方式

1. 核心选型原则：静态批量数据用「CSV」，全局固定值用「用户定义的变量」，动态随机值用「内置函数」，数据库数据用「JDBC」，复杂逻辑用「Groovy 脚本」，接口依赖用「关联参数化」

   高频组合：CSV（账号）+ 函数（订单号）+ 关联（token）是接口测试 / 压测的标配

2. 避坑点：CSV 文件编码选 UTF-8，函数参数含逗号需转义（\,），数据库驱动版本需与数据库匹配

###### 5. jmeter常用断言方式

1. 核心断言选型：通用验证用「响应断言」，JSON 精准验证用「JSON 断言」，性能验证用「持续时间 / 大小断言」，复杂逻辑用「BeanShell 断言」
2. 配置原则：只验证核心业务字段，避免过度断言
3. 失败排查：通过「查看结果树→断言结果」定位具体失败原因
4. 组合策略：全局兜底（响应断言）+ 精准验证（JSON 断言）+ 性能验证（持续时间断言）是接口测试的标配
...
自动化测试的核心局限性在于:它是==预设逻辑的执行者==,而非主动思考的测试人
它的价值体现在重复、稳定、大规模的测试场景，但无法替代手工测试在探索性、体验性、复杂逻辑测试中的核心作用
用机器的高效、稳定、重复特性，解决手工测试的低效、易出错、覆盖有限问题

建设:
确定目标范围,优先覆盖哪些核心点,调研技术方案和框架,结合团队现有现状进行选择,熟悉业务和页面结果,搭建项目基础框架,设计测试用例数据驱动和页面对象模型,加入日志、配置管理报告,持续迭代优化
...
@allure.title(): 用于设置测试用例的标题
@allure.description(): 为测试用例提供详细的描述
@allure.feature(): 标记测试用例所属的功能特性
@allure.story(): 描述测试用例所对应的用户故事
@allure.severity(): 设置测试用例的严重级别，例如 CRITICAL、NORMAL、MINOR 等
@allure.link(): 添加与测试用例相关的链接
@allure.issue(): 关联测试用例的问题或缺陷跟踪编号
@allure.testcase(): 提供测试用例的外部链接

「WebHook 地址」核心是**一个可被外部系统触发的 HTTP 接口地址**,用于实现系统间的实时消息推送(比如测试结果通知、订单状态变更、代码提交触发部署等)

#### 12.4 UI自动化

###### 12.4.1 PO模式

「**页面与用例分离、元素与操作封装**」,将每个页面抽象为一个独立的「页面类」,测试用例仅调用页面类的方法完成业务操作,而非直接操作页面元素

BDD(Behavior Driven Develoment)  行为驱动开发

###### 12.4.2 常用元素定位

###### 12.4.3 常用元素定位失败原因

元素定位不稳定是UI自动化最常见的问题之一

1、页面动态加载导致的时序问题,元素没有加载完成
2、定位策略问题
3、元素在iframe中
4、元素被遮挡/不可见
...
+ 稳定 mokey
+ Ai
  + coze dify chatflow生成测试用例
  + 根据接口文档生成接口测试用例，借用apifox
  + autogen playwright-mcp
  +  cursor/rules/english_language_check.mdc的检查标准，评估网站英文界面
...
sudo xattr -r -d com.apple.quarantine /Applications/WinCode.app
