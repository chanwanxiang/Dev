...
+ 相对路径定位
  ...
  //div/input	 	定位<div>下的直接<input>子元素

  //form//input	  <form>后代中的所有<input>（跨层级）
...
pytest playwright实现分期信息录入表单功能，包含字段输入(fill)、下拉选择(page.locator(li:visible).filter().click())、文件上传(page.expect_file_chooser())的自动化实现
...
STAR: Situation[场景]、Task[内容]、Action[动作]、Result[结果]

RSA: Read、Search、Ask

眼不见心不乱 平常修为，入一切相破一切相 方得自在
