...
+ 正则匹配
  + 一个接口返回内容较多，并且有一定规律
+ 数据库匹配核对
  + 比如对查询一个接口返回的数据进行校验时，可通过编写sql语句查询结果，然后将sql语句执行后数据库返回的结果与接口返回的结果进行对比，以此判定测试用例是否执行成功
+ 通过相关接口进行辅助验证
  + 删除接口之后执行查询接口，验证删除是否无法查询

##### 5.4.7 压力测试流程

新建测试计划创建线程组 - > 添加采样器 -> 调试接口(请求类型、请求路径、请求参数、响应结果、断言设计) -> 添加聚合报告 -> 执行脚本  -> 分析报告 

接口关联(后置处理器 ->正则或json提取器)
配置元件(配置元件 -> csv数据文件设置)
...
![image-20210917141429741](https://cdn.jsdelivr.net/gh/chanwanxiang/imageHosting/img/image-20210917141429741.png)

+ backups:脚本备份目录,格式jmx
+ bin:存放jmeter启动脚本,配置文件,模板文件
+ docs:离线帮助文档
+ extras:存放与第三方集成构建文件,集成Ant、jenkins
+ lib:存放jar包
+ licenses:许可证文件
...
2. 测试思路
   ...
   + 反例
     ...
     + 其他:黑名单,分页
3. 使用接口测试工具执行
4. Jmeter+Ant+Git+Jenkins持续集成输出测试报告

**jmeter跨线程组取参数值的方法**

在jmeter中，属性是全局的可以动态设置的，而变量是独属于每个线程组的

1. 定义属性法

   + 第一个线程组调用一个接口，正则提取想要的值到一个变量中
   + 第一个线程组添加 一个beanShell后置处理器，然后再打开菜单栏中的Tool -> 函数助手对话框选择__setProperty函数()，表达式第一个值输入将要存放的属性名称(英文)，第二个值输入你在第二步中定义的变量名称，生成函数，复制函数，将函数粘贴到beanShell后置处理器的script窗口中
   + 在第二个线程组中添加一个用户参数变量，添加变量(add variable)设置变量名称，再次打开函数助手，选择__P函数在函数第一个value值文本框输入${第三步设置属性名称}生成函数，复制函数关闭弹窗，将它粘贴到添加的变量的值文本框中
   + 在第二个线程组中，接口参数使用定义的变量${变量名称}

   如果想要设置多个属性，可以在第三步的时候粘贴多个函数，修改函数中属性名称和变量名称，每个函数之间使用;隔开

**jmeter接口关联**

1. 使用正则表达式提取器实现接口关联(可以作用于任意返回值) accessToken : (.*)?
2. 使用JsonPath表达式实现接口关联(只能作用域json返回值)  $.accessToken(绝对路径) $..accessToken(相对路径)

**jmeter动态参数处理**

函数助手对话框 ${__Random(1000,9999,随机4位数字)}
...
性能测试包括负载测试、强度测试、容量测试

负载测试指数据在超负荷环境中运行，程序是否能够承担

强度测试指它在系统资源特别低的情况下软件系统运行情况是否正常

容量测试确定系统可处理同时在线的最大用户数

**性能测试指标**

+ 并发用户数:用来度量服务器并发容量和同步协调能力,在客户端指一批用户同时执行一个操作,并发数反映了软件系统并发处理能力
+ 响应时间:客户端发出请求到得到响应的整个过程所经历的时间
  + 响应时间 = 客户端发起请求 + 网络传输时间 + 服务器处理时间
+ 吞吐量:指单位时间内系统处理的客户请求的数量,直接体现软件系统的性能承载能力
+ 资源利用率:只系统资源的使用程度,比如服务器CPU利用率、内存利用率、磁盘利用率、网络带宽利用率、jvm利用率等
...
1) 采用bash或sh+脚本的相对路径或绝对路径(不用用赋予脚本+x权限)

```bash
bash ./hello.sh
bash /home/scripts/hello.sh
```

2. 采用输入脚本的绝对路径或者相对路径脚本(必须具有可执行权限+x)
...
本质:在当前shell中打开一个子shell来执行脚本内容,当脚本内容结束,则子shell关闭,回到父shell中,开子shell与不开子shell的区别就在于,环境变量的继承关系,如在子shell中设置的当前变量,父shell是不可见的
...
2. 自定义变量
   ...
   > 变量定义规则
   >
   > 1)变量名称可以由字母、数字和下划线组成,但是不能以数字开头,环境变量名称建议大写
   >
   > 2)等号两侧不能有空格
   >
   ...
   >
   > 5)使用export可将局部变量提升为全局变量,以供其他shell程序使用
   >
   > ```bash
   > export 变量名
   > ```
...
3. 特殊变量
   ...
   ```bash
   #!/bin/bash
   # 单引号中$符号不会被识别未变量表示符,可以直接输出
   echo '====$n===='
   echo script name:$0
   echo 1st para:$1
   echo 2nd para:$2
   
   bash para.sh abc def
   ```
   ...
   > 基本语法
   >
   > 获取所有输入参数个数,常用语循环,判断参数的个数是否正确以及加强脚本的健壮性
...
分类:
	...
	条件运算符:
		> < = != <> >= <= <=>安全等于
		mysql中任何与Null作比较的都是是Null，常用来判断是否为Null
	2. 逻辑表达式筛选
	逻辑运算符:
		&& || !
		and or not
	...
	4. is null
		...
		<=>:既可以判断null值又可以判断数值
	5. IFNULL
		1. 判断第一个表达式是否为NULL，如果为NULL则返回第二个参数的值，如果不为NULL则返回第一个参数的值
...
# 截取从指定索引处后面所有字符
SELECT SUBSTR('李莫愁爱上了陆展元',7) AS output; 输出结果陆展元
# 截取从指定索引处指定字符长度的字符
SELECT SUBSTR('李莫愁爱上了陆展元',1,3) AS output; 输出结果李莫愁
...
##### 10.3.2 缓存穿透、缓存击穿、缓存雪崩

 + 缓存处理流程
   + 前台请求，后台先从缓存中获取数据，取到直接返回结果，取不到从数据库中取，数据库取到更新缓存，并返回结果，数据库也没取到，直接返回空结果
 + 缓存穿透
   + 缓存穿透是指缓存和数据库中都没有数据，而用户不断发起请求，如发起id为-1的数据或id特别大不存在的数据，这是用户很可能是攻击者，攻击会导致数据库压力过大
 + 缓存击穿
   + 缓存击穿是指缓存中没有但数据库中有的数据(一般是缓存时间到期)，这是由于并发用户特别多，同时读缓存又没读到数据，又同时去数据库取数据，引起数据库压力瞬间增大，造成过大压力
 + 缓存雪崩
   + 缓存雪崩是指缓存中大批量到过期时间，而查询量巨大，引起数据库压力过大甚至down机。和缓存击穿不同的是，缓存击穿指并发查同一条数据，缓存雪崩是不同数据都过期了，很多数据都查不到从而查数据库
...
11.10.2 其他
	...
	1. 对于一个有序数组(已按升序排序)，我们通常采用二分查找的方法来定位某一元素，请编写二分查找算法，在数组中查找指定元素。给定一个整数数组A以及他的大小n，同时给定要查找的元素val，请返回她在数组中的位置(从0开始)，若不存在该元素，返回-1。若该元素出现多次，请返回第一次出现的位置。
	1. 有一幢楼，我们现在处于18层，通过电梯，去第1层，不过楼梯比较特殊，只有2个按钮，1、2分别代表下降一层、下降两层，请问一共有多少方案去1层。
...
web端即是银行坐席,包含登录、面签受理、面签审核、订单详情、数据大屏等模块
App端即是业务员端,包含登录、订单基本信息录入、客户影像证件录入、订单列表、面签列表面签等模块
